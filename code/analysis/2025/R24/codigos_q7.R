...
config <- list(
  ...
  campo_salida = "codigo",
  tema = "Mejora Presupuesto Educativo/Recursos",
  contexto = "respuestas a la pregunta: \"¿Qué te parece que se podría hacer para que disminuyan estos hechos de violencia en las escuelas?\"",  # contexto actualizado
  ejemplo_salida = "Mejora Presupuesto Educativo/Recursos",
  chunk_size = 20L
)
...
# guardar y resumen
outfile <- paste0(config$base_path, "/analysis/", config$round, "_", config$pregunta, ".csv")
