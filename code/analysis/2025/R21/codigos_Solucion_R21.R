...
config <- list(
  year = 2025,
  round = "R21",
  pregunta = "Solucion_R21",
  base_path = "data/processed",
  campo_salida = "codigo",
  tema = "solucion_problema",
  contexto = "respuestas a la pregunta \"¿Cómo piensas que las políticas públicas del gobierno podrían ayudarte con el problema que tuviste esta semana?\"",  # contexto actualizado
  ejemplo_salida = "Apoyo Docentes",
  chunk_size = 20L
)

# datos
raw <- readxl::read_xlsx(paste0(config$base_path, "/analysis/", config$year, "/R21/emergentes_join.xlsx"))
...
# guardar y resumen
outfile <- paste0(config$base_path, "/analysis/", config$round, "_", config$tema, "_", config$pregunta, ".csv")
