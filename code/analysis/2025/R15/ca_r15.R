...
build_ca_biplot <- function(matriz, font_family = "montserrat") {
  ...
  # Número de dimensiones posibles en CA
  ndim_posible <- min(nrow(matriz) - 1, ncol(matriz) - 1)
  # Algunas matrices pueden tener sólo 0 o 1 dimensión útil
...
run_ca_for_questions <- function(s, qs = paste0("q", 1:12), out_dir = paste0("plots/", year, "/", round_id, "/")) {
  ensure_dir(out_dir)
  stops <- setdiff(stopwords("spanish"), "no")

  # Usar solo columnas de preguntas que existan en 's'
  ...
  for (q in qs_presentes) {
    ...
    textos <- s[[q]]
    if (length(textos) == 0) {
      message(q, ": columna vacía; se omite.")
      next
    }
    textos[is.na(textos)] <- ""
    ...
    # Agrupar por el docvar 'group' agregado al corpus
    ...
    outfile <- paste0(out_dir, "/ca_r15_", q, ".png")
