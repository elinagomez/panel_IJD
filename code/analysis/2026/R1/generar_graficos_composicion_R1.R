...
infile <- if (length(args) >= 1) {
  ...
  file.path("data", "processed", "transcriptions", "output", "2026", "transcripcion_R1.csv")
}

data_outdir <- if (length(args) >= 2) {
  args[[2]]
} else {
  file.path("data", "processed", "analysis", "2026", "R1")
}

plots_dir <- if (length(args) >= 3) {
  args[[3]]
} else {
  file.path("plots", "2026", "R1")
}
...
plot_segmento <- function(data, outfile) {
  ...
  p <- ggplot(tbl, aes(x = segmento, y = n, fill = segmento)) +
    ...
    labs(
      ...
      subtitle = paste0("R1 2026 | n = ", nrow(data))
    ) +
...
write.csv(summary_tbl, file.path(data_outdir, "r1_composicion_summary.csv"), row.names = FALSE)

segmentos_tbl <- df |>
  count(segmento, sort = TRUE) |>
  mutate(pct = n / sum(n))
write.csv(segmentos_tbl, file.path(data_outdir, "r1_segmentos_composicion.csv"), row.names = FALSE)
