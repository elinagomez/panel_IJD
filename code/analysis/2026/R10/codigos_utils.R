...
texto_omitido <- function(texto, config) {
  skip_values <- config$skip_values %||% character()

  if (length(skip_values) == 0 || texto_no_util(texto)) {
    return(FALSE)
  }

  texto_limpio <- trimws(as.character(texto[[1]]))
  texto_limpio %in% skip_values
}
  ...
  instrucciones <- c(
    ...
    if (!is.null(config$pregunta_cerrada)) {
      paste0(
        ...
        "respuestas abiertas breves o elipticas, pero los codigos deben clasificar la respuesta abierta."
      )
    },
    instrucciones_extra
  )
...
run_multicode_coding <- function(config) {
  ...
  infile <- config$transcription_file %||% file.path(
  ...
  clasificar_texto <- function(texto_abierto, respuesta_cerrada = NULL) {
    if (texto_omitido(texto_abierto, config)) {
      return(NA_character_)
    }
  ...
  respuestas_omitidas <- map_lgl(raw[[config$pregunta]], texto_omitido, config = config)
  respuestas_utiles <- !map_lgl(raw[[config$pregunta]], texto_no_util) & !respuestas_omitidas
  ...
  cat("respuestas omitidas por filtro: ", sum(respuestas_omitidas), "\n", sep = "")
