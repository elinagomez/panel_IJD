...
config <- list(
  ...
  output_xlsx = file.path("data", "processed", "analysis", "2026", "R10", "R10_tablas_contingencia.xlsx")
)
...
question_specs <- list(
  q1 = list(
    ...
    tipo = "abierta",
    row_header = "Código",
    source = file.path(config$analysis_dir, "codigos_q1.csv"),
    codebook = file.path(config$analysis_dir, "codigos_q1.csv")
  ),
  q2 = list(
    var = "codigos_q2",
    tipo = "abierta",
  ...
  q3 = list(
    ...
    source = "Opciones de la pauta R10",
    niveles = c(
      "Si, estoy muy informado",
      "Si, estoy algo informado",
      "Escuché del programa pero no sé de que se trata",
  ...
  q5 = list(
    ...
    codebook = file.path(config$analysis_dir, "codigos_q5.csv")
  ),
  q6 = list(
    var = "codigos_q6",
    tipo = "abierta",
  ...
  q7 = list(
    ...
    codebook = file.path(config$analysis_dir, "codigos_q7.csv")
  ),
  q8 = list(
    var = "q8",
    tipo = "cerrada",
    row_header = "Respuesta",
    source = "Opciones de la pauta R10",
    niveles = c("Sí", "Más o menos", "No", "No lo sé")
  ),
  q9 = list(
    var = "q9",
    tipo = "cerrada",
  ...
  q12 = list(
    ...
    codebook = file.path(config$analysis_dir, "codigos_q12.csv")
  )
)
...
definitions_matrix <- function(spec, q) {
  if (spec$tipo == "abierta") {
    ...
    return(
      bind_rows(
        tibble(codigo = paste0("Códigos - ", q), descripcion = NA_character_),
...
for (q in names(question_specs)) {
  ...
  writeData(wb, q, paste0("R10 2026 - ", q), startRow = 1, startCol = 1, colNames = FALSE)
